...
from shelfmark.integrations.audiobookshelf.client import abs_create_user, abs_user_exists, abs_get_user
...
def abs_copy_user_info(user_info: dict[str, Any]) -> dict[str, Any]:
    ...
    logger.debug("Audiobookshelf ABS_AUTO_PROVISION_USER is %s.", config.get("ABS_AUTO_PROVISION_USER", False))
    if not coerce_bool(config.get("ABS_AUTO_PROVISION_USER", False)):
        return user_info

    abs_user_info = user_info.copy()

    cfg = build_abs_config()
    if not cfg.base_url or not cfg.api_key:
        logger.warning("Audiobookshelf URL and API key are required to auto-provision users.")
        abs_user_info["allow_create"] = False
        return abs_user_info
    
    # Find ABS user
    username = abs_user_info.get("username")
    email = abs_user_info.get("email")
    abs_user = abs_get_user(cfg, username=username, email=email)
    if (abs_user is None):
        logger.info("Audiobookshelf user '%s' (%s) not found.", abs_user_info.get("username"), abs_user_info.get("email"))
        abs_user_info["allow_create"] = False
        return abs_user_info
    if (abs_user.get("isActive") is not True):
        logger.info("Audiobookshelf user '%s' (%s) found but is inactive.", abs_user_info.get("username"), abs_user_info.get("email"))
        abs_user_info["allow_create"] = False
        return abs_user_info

    # Copy ABS user info.
    logger.info("Audiobookshelf user '%s' (%s) found", abs_user_info.get("username"), abs_user_info.get("email"))
    abs_user_info["allow_create"] = True
    abs_user_info["username"] = abs_user.get("username")
    abs_user_info["display_name"] = abs_user.get("username")
    abs_user_info["email"] = abs_user.get("email")
    abs_user_info["role"] = abs_user.get("type")
